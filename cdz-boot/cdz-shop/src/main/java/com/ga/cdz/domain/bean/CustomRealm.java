...
public class CustomRealm extends BaseCustomRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        //管理后台的用户凭证是账号
        String shopLogin = statelessToken.getUsername();
        String clientDigest = statelessToken.getClientDigest();
        String key = RedisConstant.SHOP_TOKEN + shopLogin;
        //判断缓存中是否有登陆信息
        if (mRedisUtil.hasKey(key)) {
            ...
            return new SimpleAuthenticationInfo(shopLogin, clientDigest, this.getName());
        } else {
            //没有登录 或 过期需要重新登陆
            return null;
        }
